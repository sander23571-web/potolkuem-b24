...
const BASE_CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .data-table tfoot td { font-weight: 600; background: #f9f7ff; border-top: 2px solid var(--border); }
  ...
  .tl-dot { display: inline-block; width: 8px; height: 8px; border-radius: 50%; margin-right: 6px; background: var(--accent); flex-shrink: 0; }
...
function renderDashboard(data, allExhibitions, currentId) {
  ...
  const exhTitleHtml = exhUrl
    ? `<a href="${exhUrl}" target="_blank" style="color:inherit;text-decoration:none;border-bottom:1px dashed rgba(255,255,255,0.35)">${exhTitle}</a>`
    : exhTitle;
  ...
  // Revenue by host — keyed by host ID to preserve link info
  const revenueByHost = {};
  for (const d of deals) {
    ...
    if (!revenueByHost[hid]) revenueByHost[hid] = { name, amount: 0 };
    revenueByHost[hid].amount += parseFloat(d.OPPORTUNITY || 0);
  }
  ...
  // ── Chart data ────────────────────────────────────────────────────────────────
  const hostChartLabels = JSON.stringify(hostRevEntries.map(([, v]) => v.name));
  const hostChartData   = JSON.stringify(hostRevEntries.map(([, v]) => Math.round(v.amount)));
  const hostChartColors = JSON.stringify(hostRevEntries.map((_, i) => PALETTE[i % PALETTE.length]));
  const hostChartUrls   = JSON.stringify(hostRevEntries.map(([hid]) =>
    hid === '__employee__'
      ? `${b24Url}/crm/type/1048/details/${exhibition.id}/`
      : `${b24Url}/crm/type/1052/details/${hid}/`
  ));
  ...
  // ── Table rows ────────────────────────────────────────────────────────────────
  const maxExpAmt = Math.max(...expenses.map(x => parseFloat(x.ufCrm24Amount || 0)), 1);
  ...
  const shiftRows = shiftDateKeys.flatMap(dt =>
    shiftsByDate[dt].map(s => {
      ...
      const hostLink = hid
        ...
        : hostName;
      const shiftUrl = `${b24Url}/crm/type/1056/details/${s.id}/`;
  ...
  <div class="section">
    ...
    <div class="two-col">
      <div class="chart-card">
        <h3>Выручка по ведущим <span class="chart-hint">(кликните — карточка в Б24)</span></h3>
        <canvas id="hostChart" height="220"></canvas>
      </div>
      <div class="chart-card">
        <h3>Структура расходов <span class="chart-hint">(кликните — открыть расход)</span></h3>
...
// Chart 2 — Структура расходов (пончик, клик → карточка расхода в Б24)
...
new Chart(document.getElementById('expenseChart'), {
  ...
  options: {
    responsive: true,
    cutout: '60%',
    onClick: (e, els) => { if (els.length) window.open(expUrls[els[0].index], '_blank'); },
    onHover:  (e, els) => { e.native.target.style.cursor = els.length ? 'pointer' : 'default'; },
    plugins: {
...
function renderComparison(allExhibitions, summaries) {
  ...
  const totalRevAll  = sorted.reduce((s, e) => s + summaries[e.id].revenue,  0);
  const totalExpAll  = sorted.reduce((s, e) => s + summaries[e.id].expenses, 0);
  const totalPnlAll  = totalRevAll - totalExpAll;
  const totalDeals   = sorted.reduce((s, e) => s + summaries[e.id].dealCount, 0);
  const pnlAllClass  = totalPnlAll >= 0 ? 'green' : 'red';
  const pnlAllSign   = totalPnlAll >= 0 ? '+' : '−';

  const cmpLabels   = JSON.stringify(sorted.map(e => e.title || `#${e.id}`));
  const cmpRevenues = JSON.stringify(sorted.map(e => Math.round(summaries[e.id].revenue)));
  const cmpExpenses = JSON.stringify(sorted.map(e => Math.round(summaries[e.id].expenses)));
  const cmpUrls     = JSON.stringify(sorted.map(e => `/report/${e.id}`));

  const tableRows = sorted.map(e => {
    const s       = summaries[e.id];
    const pnl     = s.revenue - s.expenses;
    const sign    = pnl >= 0 ? '+' : '−';
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Все выставки — Потолкуем?</title>
<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"></script>
<style>${BASE_CSS}</style>
</head>
<body>
...
<div class="hero">
  <div class="hero-label">Сравнение · Настольные игры «Потолкуем?»</div>
  <h1>Все выставки</h1>
  <div class="hero-dates">
...
<div class="container">
  ...
  <!-- TOTAL KPI -->
  <div class="section">
    <div class="section-title">Итого по всем выставкам</div>
  ...
  <!-- COMPARISON CHART -->
  <div class="section">
    <div class="section-title">Выручка vs Расходы <span class="chart-hint">(кликните на столбец — дашборд выставки)</span></div>
    <div class="chart-card">
  ...
  <!-- TABLE -->
  <div class="section">
    <div class="section-title">Сводная таблица</div>
    <table class="data-table">
      <thead>
        <tr>
          <th>Выставка</th>
...
<div class="footer">
  Потолкуем? · Сравнение выставок · potolkuem.bitrix24.ru
</div>
...
new Chart(document.getElementById('cmpChart'), {
  ...
  data: {
    labels: ${cmpLabels},
    datasets: [
      { label: 'Выручка', data: ${cmpRevenues}, backgroundColor: '#27ae60', borderRadius: 3 },
      { label: 'Расходы', data: ${cmpExpenses}, backgroundColor: '#c0392b', borderRadius: 3 }
    ]
  },
  options: {
    responsive: true,
...
module.exports = { renderDashboard, renderComparison };
